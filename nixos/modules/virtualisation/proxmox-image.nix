...
with lib;
{
  ...
  options.proxmox = {
    qemuConf = {
      ...
      efidisk0 = mkOption {
        type = types.str;
        default = "local-lvm:vm-9999-efi-0";
        example = "ceph:vm-123-efi-0";
        description = ''
          Configuration for the EFI disk. It can be used as a cue for PVE to autodetect the target storage.
          This property is only used when the guest OS requires EFI boot support.
        '';
      };
      ostype = mkOption {
        type = types.str;
  ...
  config =
    let
      ...
      efidisk0Storage = builtins.head (builtins.split ":" cfg.qemuConf.efidisk0);
      cfgFile =
        fileName: properties:
        pkgs.writeTextDir fileName ''
          ...
          ${lib.optionalString touchEFIVars "#qmdump#map:efidisk0:drive-efidisk0:${efidisk0Storage}:raw:"}
        '';
        ...
        postVM =
          ...
          ''
            ${vma}/bin/vma create "${config.image.baseName}.vma" \
              ...
              }/qemu-server.conf drive-virtio0=$diskImage ${lib.optionalString touchEFIVars "drive-efidisk0=$efiVars"}
            rm $diskImage
